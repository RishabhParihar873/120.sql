CREATE OR REPLACE PACKAGE BODY ict_universal_doc_engine AS
    ...
    BEGIN
        -- 1. Route template and output type based on target format
        ...
        -- 2. Prepare File Name early because AOP requires it
        l_file_name := REGEXP_REPLACE(p_title, '[^a-zA-Z0-9_]', '_') || '_' || TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MI') || l_file_ext;

        -- 3. Call APEX Office Print (AOP) - FIX: It's a Function, so we assign it to l_return_blob
        l_return_blob := aop_api_pkg.plsql_call_to_aop(
            ...
            p_output_to        => 'directory' -- 'directory' or 'none' prevents it from trying to pop up in the browser abruptly
        );

        -- 4. Store the generated document in CGCT_AI_DOCUMENT_LIBRARY
        ...
        -- 5. Construct the download link for the Agent to give to the user
        l_download_url := 'f?p=' || v('APP_ID') || ':920:' || v('APP_SESSION') || ':::P920_ID:' || l_new_doc_id;

        -- 6. Return success structured data to AI Agent
        RETURN json_object(
