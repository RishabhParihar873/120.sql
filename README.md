Here's the complete package body — copy this whole block into SQL Commands and run it:
...
1. **Run the block above** in SQL Commands.

2. **Confirm it compiled clean:**
```sql
select status from user_objects where object_name = 'ICT_UNIVERSAL_DOC_ENGINE';
```
Should say `VALID`. If it says `INVALID`, stop and send me the error — don't go further.

3. **Check page 920 actually exists** (this matters — if it doesn't, the link is pointing nowhere):
```sql
select page_id, page_name from apex_application_pages where application_id = 120 and page_id = 920;
```
If this returns **nothing**, tell me immediately — we'll need to either create that download page or change 920 to whatever page ID you actually intend to use for downloads.

4. **Test in chat:** "generate me a pdf explaining ICT"

5. **Send me everything at once** — screenshot of both query results + screenshot of the chat outcome. That way I'm not guessing between multiple possible causes again.
